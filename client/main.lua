...
function openBindingMenu()
    local keyMeta = QBX.PlayerData.metadata.commandbinds
    SendNUIMessage({
        action = "openBinding",
        keyData = keyMeta
    })
        ...
        if LocalPlayer.state.isLoggedIn and not keyPressed and GetLastInputMethod(0) then
            ...
            if next(keyMeta) ~= nil then
                if keyMeta[v[2]].command ~= "" then
                    ...
                    exports.qbx_core:Notify('There is still nothing ['..v[2]..'] bound, /binds to bind a command', 'primary', 4000)
                end
            else
                exports.qbx_core:Notify('You have not bound any commands, /binds to bind a command', 'primary', 4000)
            end
        end
    end, false)
...
RegisterNUICallback('save', function(data, cb)
    ...
    exports.qbx_core:Notify('Command bindings have been saved!', 'success')
